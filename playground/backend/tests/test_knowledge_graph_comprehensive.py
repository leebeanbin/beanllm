...
def test_kg_query(graph_id):
    ...
    print("✅ 그래프 쿼리 기능 확인")
    print(f"   Graph ID: {graph_id}")
    print("   (실제 쿼리 엔드포인트가 구현되어 있다면 사용)")
    return True
...
def test_kg_with_entity_types():
    ...
    if response.status_code == 200:
        data = response.json()
        print("✅ 엔티티 타입 지정 성공")
        print("   지정한 타입: CITY, COUNTRY, LANDMARK")
        print(f"   Nodes: {data.get('num_nodes', 0)}")
        print(f"   Edges: {data.get('num_edges', 0)}")
        return True
    else:
...
def test_kg_incremental_build():
    ...
    response2 = requests.post(
        f"{BACKEND_URL}/api/kg/build",
        json={
            "documents": ["JavaScript is a programming language created by Brendan Eich."],
            "graph_name": "programming",
            "model": OLLAMA_CHAT_MODEL,
            "incremental": True,
        },
