...
def test_conversation_context():
    ...
    if response.status_code == 200:
        ...
        if "Alice" in answer or "alice" in answer.lower():
            print(f"✅ Correctly remembered name: Alice")
        else:
            print(f"⚠️  May not have remembered the name")
        return True
    else:
        print(f"❌ 실패: {response.text}")
        return False
...
def test_system_message():
    ...
    response = requests.post(
        ...
        json={
            "messages": [
                {"role": "system", "content": "You are a helpful math tutor. Answer in one short sentence."},
    ...
    if response.status_code == 200:
        data = response.json()
        answer = data.get('response', '')
        print(f"✅ System message applied")
...
def test_think_mode():
    ...
    response = requests.post(
        ...
        json={
            "messages": [
                {"role": "user", "content": "If a train travels at 60 mph for 2 hours, how far does it go?"}
            ],
            "model": OLLAMA_CHAT_MODEL,
    ...
    if response.status_code == 200:
        ...
        if "<think>" in answer or "step by step" in answer.lower():
            ...
            print(f"⚠️  추론 과정이 명시적이지 않을 수 있음 (모델 의존적)")
        return True
    else:
        print(f"❌ 실패: {response.text}")
        return False
