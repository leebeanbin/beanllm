...
async def test_metrics_storage():
    ...
    try:
        ...
        await redis_client.setex(
            f"request:status:{request_id}", 3600, json.dumps(status_data).encode("utf-8")
        )
        print(f"✅ 요청 상태 저장 성공: request:status:{request_id}")

        # 2. 응답 시간 메트릭 저장
        await redis_client.zadd("metrics:response_time", {request_id: 100.5})
        await redis_client.expire("metrics:response_time", 3600)
        print("✅ 응답 시간 메트릭 저장 성공")
...
async def test_dashboard_data_retrieval():
    ...
    try:
        ...
        # 1. 응답 시간 메트릭
        response_times_raw = redis_client.zrange("metrics:response_time", 0, -1, withscores=True)
