...
def test_web_search_basic():
    ...
    response = requests.post(
        ...
        json={"query": "Python programming language", "engine": "duckduckgo", "max_results": 5},
    )
...
def test_web_search_different_queries():
    ...
    queries = ["machine learning", "artificial intelligence tutorial", "best programming practices"]
    ...
    for query in queries:
        response = requests.post(
            f"{BACKEND_URL}/api/web/search",
            json={"query": query, "engine": "duckduckgo", "max_results": 3},
        )
...
def test_web_search_max_results():
    ...
    response = requests.post(
        f"{BACKEND_URL}/api/web/search",
        json={"query": "web development", "engine": "duckduckgo", "max_results": 10},
    )
    ...
    if response.status_code == 200:
        ...
        if len(results) > 0:
            print("✅ 검색 성공 (엔진 제한으로 인해 요청보다 적을 수 있음)")
        return True
    else:
        print(f"❌ 실패: {response.text}")
        return False
...
def test_web_search_and_scrape():
    ...
    response = requests.post(
        f"{BACKEND_URL}/api/web/search",
        json={"query": "OpenAI GPT", "engine": "duckduckgo", "max_results": 3},
    )
    ...
    if response.status_code == 200:
        ...
        print("   (스크래핑 엔드포인트가 별도로 있다면 추가 테스트 필요)")
...
def test_web_search_empty_query():
    ...
    response = requests.post(
        f"{BACKEND_URL}/api/web/search",
        json={"query": "", "engine": "duckduckgo", "max_results": 5},
    )

    print(f"Status: {response.status_code}")
    if response.status_code >= 400:
        print("✅ 올바르게 에러 처리 (빈 쿼리는 거부되어야 함)")
        return True
...
def test_web_search_special_characters():
    ...
    response = requests.post(
        f"{BACKEND_URL}/api/web/search",
        json={"query": "C++ programming & algorithms", "engine": "duckduckgo", "max_results": 5},
    )
    ...
    if response.status_code == 200:
        ...
        results = data.get("results", [])
        print("✅ 특수 문자 처리 완료")
        print("   Query: C++ programming & algorithms")
        print(f"   Results: {len(results)}개")
        return True
    else:
        print(f"❌ 실패: {response.text}")
