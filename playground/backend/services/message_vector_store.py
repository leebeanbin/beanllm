...
class MessageVectorStore:
    ...
    async def save_message(
        ...
        try:
            ...
            # 임베딩 모델 미설치(nomic-embed-text 등) 시 선택적 기능 실패로 간주
            logger.warning("Failed to save message to vector DB: %s", e)
            return False

    @staticmethod
    async def get_session_messages(
        session_id: str, limit: Optional[int] = None, role: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            results = await asyncio.to_thread(
                _message_vector_store.collection.get, where=where_filter, limit=limit
            )
            ...
            logger.debug(
                f"✅ Retrieved {len(messages)} messages from vector DB for session: {session_id}"
            )
            return messages
    ...
    async def search_messages(
        query: str, session_id: Optional[str] = None, role: Optional[str] = None, k: int = 20
    ) -> List[Dict[str, Any]]:
        """
        메시지 내용으로 검색 (의미 기반)
    ...
    async def delete_session_messages(session_id: str) -> bool:
        ...
        try:
            ...
            results = await asyncio.to_thread(
                _message_vector_store.collection.get, where={"session_id": session_id}
            )

            if results["ids"]:
                # 모든 메시지 삭제 (동기 함수이므로 asyncio.to_thread 사용)
                await asyncio.to_thread(_message_vector_store.collection.delete, ids=results["ids"])
                logger.debug(
                    f"✅ Deleted {len(results['ids'])} messages from vector DB for session: {session_id}"
                )
    ...
    async def delete_message(message_id: str) -> bool:
        ...
        try:
            ...
            await asyncio.to_thread(_message_vector_store.collection.delete, ids=[message_id])
