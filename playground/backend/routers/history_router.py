...
async def list_sessions(
    ...
    try:
        ...
        if query:
            ...
            logger.info(f"✅ Searched sessions: {result['search_type']} search, {len(result['sessions'])} results")
            return SessionListResponse(sessions=result["sessions"], total=result["total"])
        ...
        if cached:
            logger.debug(f"✅ Session list cache hit")
        ...
        # Get sessions
        cursor = db.chat_sessions.find(mongo_query).sort(sort_field, sort_direction).skip(skip).limit(limit)
        sessions = []

        async for doc in cursor:
            doc["_id"] = str(doc["_id"])
            # ✅ message_count는 이미 저장되어 있으므로 사용 (O(1))
...
async def get_session(session_id: str):
    ...
    try:
        ...
        if cached:
            ...
            # 캐시된 메시지는 요약만 포함되므로 Vector DB에서 실제 메시지 가져오기
        ...
        # ✅ Vector DB에서 실제 메시지 내용 가져오기
        from services.message_vector_store import message_vector_store
        messages = await message_vector_store.get_session_messages(session_id)
        
        # 메시지가 있으면 Vector DB에서 가져온 것으로 교체
        if messages:
            session["messages"] = messages
        # Vector DB에 없으면 MongoDB의 요약 정보 사용 (fallback)
...
async def add_message(session_id: str, request: AddMessageRequest):
    ...
    try:
        ...
        message_summary = {
            "message_id": message_id,
            "role": request.role,
            "content_preview": request.content[:100] if len(request.content) > 100 else request.content,  # 미리보기만
            "timestamp": now,
            "model": request.model,
        }
        ...
        logger.info(f"✅ Added message to session: {session_id} (role: {request.role}, message_id: {message_id})")
