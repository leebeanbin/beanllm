...
def print_quick_start():
    ...
    OnboardingPattern.render(
        ...
        steps=[
            ...
            {"title": "Try it out", "description": "from beanllm import Client; client = Client()"},
            {"title": "Use CLI", "description": "beanllm list"},
            {"title": "Read docs", "description": "https://github.com/leebeanbin/beanllm"},
        ],
    )
...
def print_providers_status():
    ...
    console = get_console()
    table = Table(
        title="[bold cyan]📦 Provider Status[/bold cyan]", box=box.ROUNDED, show_header=True
    )
