...
class GraphServiceImpl(IGraphService):
    ...
    @with_distributed_features(
        ...
        rate_limit_key=lambda self, args, kwargs: f"graph:node:{(args[0] if args else kwargs.get('request')).entry_point or 'default'}",
        lock_key=lambda self, args, kwargs: f"graph:node:{hash(str((args[0] if args else kwargs.get('request')).nodes)) if hasattr(args[0] if args else kwargs.get('request'), 'nodes') else 'default'}",
