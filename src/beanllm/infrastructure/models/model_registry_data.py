"""
Model Registry Data - 지원 모델 정의

모든 LLM Provider의 모델 메타데이터를 중앙 관리.
모델 추가/수정 시 이 파일만 변경하면 됨.
"""

from typing import Dict

from beanllm.infrastructure.models.llm_provider import LLMProvider
from beanllm.infrastructure.models.model_config import ModelConfig
...
MODELS: Dict[str, ModelConfig] = {
    ...
    # ================================================================
    "phi3.5": ModelConfig(
        name="phi3.5", display_name="Phi-3.5 (SLM)",
        provider=LLMProvider.OLLAMA, type="slm", max_tokens=2048, temperature=0.0,
        description="빠른 응답을 위한 Small Language Model",
        use_case="간단한 질문, 검색 제안, 자동완성",
    ),
    "qwen2.5:7b": ModelConfig(
        name="qwen2.5:7b", display_name="Qwen2.5 7B (LLM)",
        provider=LLMProvider.OLLAMA, type="llm", max_tokens=4096, temperature=0.0,
        description="균형잡힌 성능의 Large Language Model",
        use_case="일반 대화, 설명, 분석",
    ),
    "llama3.1:70b": ModelConfig(
        name="llama3.1:70b", display_name="Llama 3.1 70B (Large LLM)",
        provider=LLMProvider.OLLAMA, type="llm", max_tokens=8192, temperature=0.0,
        description="고성능 추론을 위한 Large Language Model",
        use_case="복잡한 분석, 전략 수립, 심층 추론",
    ),
    "ax:3.1-lite": ModelConfig(
        name="ax:3.1-lite", display_name="A.X 3.1 Lite (Korean)",
        provider=LLMProvider.OLLAMA, type="llm", max_tokens=4096, temperature=0.0,
        description="한국어 특화 모델",
        use_case="한국어 금융 질문, 한국 시장 분석",
    ),
    "llama3.3:70b": ModelConfig(
        name="llama3.3:70b", display_name="Llama 3.3 70B",
        provider=LLMProvider.OLLAMA, type="llm", max_tokens=8192, temperature=0.0,
        description="Meta의 Llama 3.3 70B 모델 (2025)",
        use_case="고성능 추론, 복잡한 분석",
    ),
    "qwen3": ModelConfig(
        name="qwen3", display_name="Qwen 3",
        provider=LLMProvider.OLLAMA, type="llm", max_tokens=8192, temperature=0.0,
        description="Alibaba의 Qwen 3 모델 (2025)",
        use_case="다국어 지원, 코드 생성",
    ),
    "phi4": ModelConfig(
        name="phi4", display_name="Phi-4",
        provider=LLMProvider.OLLAMA, type="slm", max_tokens=4096, temperature=0.0,
    ...
    "gpt-4o-mini": ModelConfig(
        ...
        provider=LLMProvider.OPENAI, type="llm", max_tokens=16384, temperature=0.0,
        description="OpenAI의 빠르고 저렴한 모델",
        use_case="일반 대화, 빠른 응답",
    ),
    "gpt-4o": ModelConfig(
        name="gpt-4o", display_name="GPT-4o",
        provider=LLMProvider.OPENAI, type="llm", max_tokens=128000, temperature=0.0,
    ...
    "o1": ModelConfig(
        name="o1", display_name="O1",
        provider=LLMProvider.OPENAI, type="llm", max_tokens=100000, temperature=1.0,
        description="OpenAI의 추론 모델 (2024)",
        use_case="복잡한 추론, 수학, 과학, 코딩",
        supports_temperature=False,
    ),
    "o1-mini": ModelConfig(
        name="o1-mini", display_name="O1 Mini",
        provider=LLMProvider.OPENAI, type="llm", max_tokens=65536, temperature=1.0,
        description="OpenAI의 빠른 추론 모델",
        use_case="코딩, 수학, 과학 (비용 효율적)",
        supports_temperature=False,
    ),
    "o1-preview": ModelConfig(
        name="o1-preview", display_name="O1 Preview",
        provider=LLMProvider.OPENAI, type="llm", max_tokens=128000, temperature=1.0,
    ...
    "gpt-4.1": ModelConfig(
        ...
        provider=LLMProvider.OPENAI, type="llm", max_tokens=1000000, temperature=0.0,
    ...
    "gpt-4.1-mini": ModelConfig(
        ...
        provider=LLMProvider.OPENAI, type="llm", max_tokens=500000, temperature=0.0,
        description="OpenAI의 GPT-4.1 Mini 모델 (2025.04)",
        use_case="빠른 응답, 비용 효율적",
    ),
    "gpt-4.1-nano": ModelConfig(
        name="gpt-4.1-nano", display_name="GPT-4.1 Nano",
        provider=LLMProvider.OPENAI, type="llm", max_tokens=200000, temperature=0.0,
    ...
    "gpt-4.5-mini": ModelConfig(
        name="gpt-4.5-mini", display_name="GPT-4.5 Mini",
        provider=LLMProvider.OPENAI, type="llm", max_tokens=500000, temperature=0.0,
    ...
    "gpt-5": ModelConfig(
        ...
        provider=LLMProvider.OPENAI, type="llm", max_tokens=200000, temperature=1.0,
        description="OpenAI의 GPT-5 추론 모델 (2025.08)",
        use_case="최고급 추론, 복잡한 문제 해결, 코딩",
        supports_temperature=False, supports_max_tokens=False, uses_max_completion_tokens=True,
    ),
    "gpt-5.1": ModelConfig(
        name="gpt-5.1", display_name="GPT-5.1",
        provider=LLMProvider.OPENAI, type="llm", max_tokens=200000, temperature=1.0,
    ...
    "claude-3-5-sonnet-20241022": ModelConfig(
        ...
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=8192, temperature=0.0,
    ...
    "claude-3-opus-20240229": ModelConfig(
        ...
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=4096, temperature=0.0,
    ...
    "claude-opus-4": ModelConfig(
        ...
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=16384, temperature=0.0,
        description="Anthropic의 Claude 4 Opus 모델 (2025)",
        use_case="최고 수준 추론, 복잡한 분석",
    ),
    "claude-sonnet-4": ModelConfig(
        name="claude-sonnet-4", display_name="Claude Sonnet 4",
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=16384, temperature=0.0,
        description="Anthropic의 Claude 4 Sonnet 모델 (2025)",
        use_case="균형잡힌 성능, 일반 작업",
    ),
    "claude-haiku-4": ModelConfig(
        name="claude-haiku-4", display_name="Claude Haiku 4",
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=16384, temperature=0.0,
        description="Anthropic의 Claude 4 Haiku 모델 (2025)",
        use_case="빠른 응답, 비용 효율적",
    ),
    "claude-opus-4-1": ModelConfig(
        name="claude-opus-4-1", display_name="Claude Opus 4.1",
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=32768, temperature=0.0,
        description="Anthropic의 Claude 4.1 Opus 모델 (2025)",
        use_case="최고급 추론, 장문 생성",
    ),
    "claude-sonnet-4-1": ModelConfig(
        name="claude-sonnet-4-1", display_name="Claude Sonnet 4.1",
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=32768, temperature=0.0,
    ...
    "claude-opus-4-5": ModelConfig(
        ...
        provider=LLMProvider.ANTHROPIC, type="llm", max_tokens=64000, temperature=0.0,
    ...
    "gemini-2.0-flash-exp": ModelConfig(
        ...
        provider=LLMProvider.GOOGLE, type="llm", max_tokens=8192, temperature=0.0,
    ...
    "gemini-2.5-pro": ModelConfig(
        ...
        provider=LLMProvider.GOOGLE, type="llm", max_tokens=65536, temperature=0.0,
    ...
    "gemini-3.0-pro": ModelConfig(
        name="gemini-3.0-pro", display_name="Gemini 3.0 Pro",
        provider=LLMProvider.GOOGLE, type="llm", max_tokens=65536, temperature=0.0,
        description="Google의 Gemini 3.0 Pro 모델 (2025)",
        use_case="최첨단 성능, 복잡한 추론",
    ),
    "gemini-3.0-deep-think": ModelConfig(
        name="gemini-3.0-deep-think", display_name="Gemini 3.0 Deep Think",
        provider=LLMProvider.GOOGLE, type="llm", max_tokens=65536, temperature=0.0,
    ...
    "deepseek-chat": ModelConfig(
        ...
        provider=LLMProvider.DEEPSEEK, type="llm", max_tokens=8192, temperature=0.0,
    ...
    "deepseek-v3-0324": ModelConfig(
        name="deepseek-v3-0324", display_name="DeepSeek V3 (2025.03.24)",
        provider=LLMProvider.DEEPSEEK, type="llm", max_tokens=8192, temperature=0.0,
        description="DeepSeek V3 모델 (2025.03.24)",
        use_case="일반 대화, 코드 생성",
    ),
    "deepseek-v3-1": ModelConfig(
        name="deepseek-v3-1", display_name="DeepSeek V3.1",
        provider=LLMProvider.DEEPSEEK, type="llm", max_tokens=8192, temperature=0.0,
        description="DeepSeek V3.1 모델 (2025)",
        use_case="향상된 성능, 코드 생성, 분석",
    ),
    "deepseek-v3-2": ModelConfig(
        name="deepseek-v3-2", display_name="DeepSeek V3.2",
        provider=LLMProvider.DEEPSEEK, type="llm", max_tokens=8192, temperature=0.0,
    ...
    "deepseek-r1-0528": ModelConfig(
        name="deepseek-r1-0528", display_name="DeepSeek R1 (2025.05.28)",
        provider=LLMProvider.DEEPSEEK, type="llm", max_tokens=8192, temperature=0.0,
    ...
    # ================================================================
    "sonar": ModelConfig(
        name="sonar", display_name="Perplexity Sonar",
        provider=LLMProvider.PERPLEXITY, type="llm", max_tokens=4096, temperature=0.0,
        description="Perplexity의 실시간 웹 검색 + LLM 통합 모델 (Llama 3.3 70B 기반)",
        use_case="실시간 정보 검색, 최신 정보 질문",
    ),
    "sonar-pro": ModelConfig(
        name="sonar-pro", display_name="Perplexity Sonar Pro",
        provider=LLMProvider.PERPLEXITY, type="llm", max_tokens=4096, temperature=0.0,
        description="Perplexity의 고성능 모델 (더 정확한 검색 및 답변)",
        use_case="고품질 검색, 상세한 인용이 필요한 작업",
    ),
    "sonar-reasoning-pro": ModelConfig(
        name="sonar-reasoning-pro", display_name="Perplexity Sonar Reasoning Pro",
        provider=LLMProvider.PERPLEXITY, type="llm", max_tokens=4096, temperature=0.0,
