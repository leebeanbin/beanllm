...
def run_improvement_cycle_step(loop: Any, improvement_plan: Optional[ImprovementPlan] = None) -> ImprovementCycle:
    ...
    if improvement_plan is None:
        ...
        if not plans:
            cycle = ImprovementCycle(cycle_number=cycle_number, timestamp=datetime.now(timezone.utc), chunking_result=None, eval_score_before=score_before, eval_score_after=score_before, improvement=0.0, strategy_used="none", changes_made=[])
    ...
    if improvement_plan.area == "chunking" and improvement_plan.config_changes:
        ...
        chunking_result = loop.chunking_experimenter.run_experiment(config=new_config, strategy_name=f"improved_cycle_{cycle_number}")
    ...
    cycle = ImprovementCycle(cycle_number=cycle_number, timestamp=datetime.now(timezone.utc), chunking_result=chunking_result, eval_score_before=score_before, eval_score_after=score_after, improvement=score_after - score_before, strategy_used=improvement_plan.action[:50], changes_made=changes_made)
    loop._cycles.append(cycle)
    logger.info(f"Improvement cycle {cycle_number}: {score_before:.4f} -> {score_after:.4f} (+{cycle.improvement:.4f})")
    return cycle


def run_full_cycle(loop: Any, max_iterations: int = 3, target_improvement: float = 0.2) -> Dict[str, Any]:
    ...
    return {"initial_score": initial_score, "final_score": final_score, "total_improvement": total_improvement, "cycles_run": len(loop._cycles), "best_config": loop._current_best_config, "cycles": loop._cycles}
