...
class ModelRegistry:
    ...
    def _scan_providers(self):
        ...
        for name, env_key, env_value in provider_configs:
            try:
                ...
                self._providers[name] = ProviderInfo(name=name, status=status, env_key=env_key, env_value_set=bool(env_value), available_models=available_models, default_model=default_model)
            except Exception as e:
                logger.error(f"Error scanning provider {name}: {e}")
                self._providers[name] = ProviderInfo(name=name, status=ModelStatus.ERROR, env_key=env_key, env_value_set=bool(env_value), error_message=str(e))
    ...
    def _scan_models(self):
        ...
        for model_name, model_config in all_models.items():
            try:
                ...
                parameters.append(ParameterInfo(name="temperature", type="float", description="응답의 창의성/랜덤성 조절 (0.0-2.0)", default=default_temp, required=False, supported=supports_temp, notes="일부 모델(gpt-5-mini, o3 등)은 temperature 미지원" if not supports_temp else None))
                ...
                if uses_max_completion:
                    parameters.append(ParameterInfo(name="max_completion_tokens", type="int", description="생성할 최대 토큰 수 (새로운 모델용)", default=default_max_tokens, required=False, supported=supports_max_tokens, notes="새로운 모델(gpt-5, gpt-4.1 시리즈)은 max_completion_tokens 사용"))
                else:
                    parameters.append(ParameterInfo(name="max_tokens", type="int", description="생성할 최대 토큰 수", default=default_max_tokens, required=False, supported=supports_max_tokens, notes="일부 모델은 max_tokens 미지원" if not supports_max_tokens else None))
                example_usage = self._generate_example_usage(model_name, model_config)
                self._models[model_name] = ModelCapabilityInfo(model_name=model_name, display_name=model_config.get("display_name", model_name), provider=model_config["provider"], model_type=model_config["type"], supports_streaming=True, supports_temperature=supports_temp, supports_max_tokens=supports_max_tokens, uses_max_completion_tokens=uses_max_completion, max_tokens=default_max_tokens, default_temperature=default_temp, description=model_config.get("description", ""), use_case=model_config.get("use_case", ""), parameters=parameters, example_usage=example_usage)
            except Exception as e:
    ...
    def _generate_example_usage(self, model_name: str, model_config: dict) -> str:
        ...
        env_key_map = {"openai": "OPENAI_API_KEY", "anthropic": "ANTHROPIC_API_KEY", "google": "GEMINI_API_KEY", "ollama": "OLLAMA_HOST"}
...
'''
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {"total_providers": len(self._providers), "active_providers": len(active), "total_models": len(self._models), "providers": {p.name: {"status": p.status.value, "available_models_count": len(p.available_models), "default_model": p.default_model} for p in self._providers.values()}, "active_provider_names": [p.name for p in active]}
